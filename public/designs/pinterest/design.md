...
The typography uses Pin Sans - a custom proprietary font with a broad fallback stack including Japanese fonts, reflecting Pinterest's global reach. At display scale (70px, weight 600), Pin Sans creates large, inviting headlines. At smaller sizes, the system is compact: buttons at 12px, captions at 12â14px. The CSS variable naming system (`--comp-*`, `--sema-*`, `--base-*`) reveals a sophisticated three-tier design token architecture: component-level, semantic-level, and base-level tokens.
...
- **Primary**: `Pin Sans`, fallbacks: `-apple-system, system-ui, Segoe UI, Roboto, Oxygen-Sans, Apple Color Emoji, Segoe UI Emoji, Segoe UI Symbol, Ubuntu, Cantarell, Fira Sans, Droid Sans, Helvetica Neue, Helvetica, ãã©ã®ãè§ã´ Pro W3, ã¡ã¤ãªãª, Meiryo, ï¼­ï¼³ ï¼°ã´ã·ãã¯, Arial`
...
- **Compact type scale**: The range is 12pxâ70px with a dramatic jump - most functional text is 12â16px, creating a dense, app-like information hierarchy.
- **Warm weight distribution**: 600â700 for headings, 400â500 for body. No ultra-light weights - the type always feels substantial.
