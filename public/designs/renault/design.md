...
- **Hero Aurora**: Sweeping multi-color gradients (magenta â violet â teal) applied to hero slide backgrounds - the site's most distinctive visual element. These are photographic/composited rather than CSS gradients
...
- Used for: "keÅfedin" (explore), secondary conversion actions
...
- fontSize 16px (NouvelR), fontWeight 700
- Used for: "ilk sen Ã¶Ären" (be the first to know), "satÄ±n alÄ±n" (buy), secondary actions
...
- Set minimum interactive size at 46Ã46px for all buttons - accessibility built into the component spec
- Reserve pill-shaped radius (46â50px) exclusively for search inputs and filter elements - never for buttons
...
- All buttons: minimum 46Ã46px (`minWidth: 46px, minHeight: 46px`) - exceeds WCAG AAA 44Ã44px requirement
